...
const BookRecommendationCard = ({ recommendation }: { recommendation: RecommendBookOutput }) => (
  <Card className="mt-2 shadow-md bg-card border-primary/50">
    ...
    <CardContent>
      <Image 
        src={`https://placehold.co/300x200.png?text=${encodeURIComponent(recommendation.bookTitle)}`}
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  if (isSystem) {
    return (
      <div className="text-center my-2">
        <p className="text-xs text-muted-foreground italic px-4 py-1 bg-muted rounded-full inline-block">{message.text}</p>
      </div>
    );
  }
  ...
  return (
    <div
      className={cn(
        'flex mb-4 animate-in fade-in slide-in-from-bottom-4 duration-300 ease-out',
